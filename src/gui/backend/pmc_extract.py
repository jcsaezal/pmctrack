...
class PMCExtract(object):
	def __init__(self, user_config):
		self.user_config = user_config
                ...
                # Dictionary data extracted from pmctrack command. Format: data[pid/cpu][nr_experiment][nr_metric][pos_data]
		self.data = {}

		# Character that indicates the benchmark's state: running (R), stopped (S), killed (K) or finished (F)
		self.state = 'R'
		...
		if not self.user_config.machine.is_remote:
			...
			comando.append("-t")
			comando.append(self.__get_command() + " 2>&1")
	...
	def __extract_information(self):
		...
		for line in iter(self.pipe.stdout.readline, ''):
			...
			for metric_conf in self.user_config.experiments[num_exp].metrics:
				...
				self.data[pack][num_exp][num_metric].append(metric_eval)
                                if self.out_metrics != None:
